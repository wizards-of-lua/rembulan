...
public class LogicalOperatorsBug {

  public static void main(String[] args)
      throws InterruptedException, CallException, LoaderException, CallPausedException {
    // System.setProperty("net.sandius.rembulan.compiler.VerifyAndPrint", "true");
    String program = "print( 5<=3 and 'string of success' or 'string of failure')";

    StateContext state = StateContexts.newDefaultInstance();
    Table env = StandardLibrary.in(RuntimeEnvironments.system()).installInto(state);

    ChunkLoader loader = CompilerChunkLoader.of("reproducer");
    LuaFunction main = loader.loadTextChunk(new Variable(env), "chunk", program);
